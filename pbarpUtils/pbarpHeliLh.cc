...
complex<double> pbarpHeliLh::calcSpinDensity(Spin M1, Spin M2, std::string& nameDec, EvtData* theData){
...
double pbarpHeliLh::calcSpinDensityNorm(std::string& nameDec, EvtData* theData){
...
complex<double> pbarpHeliLh::calcProdPartAmp(Spin lamX, Spin lamDec, std::string nameDec, EvtData* theData, 
...
double pbarpHeliLh::calcEvtIntensity(EvtData* theData, fitParams& theParamVal){

  double result=0.;

  std::map <boost::shared_ptr<const JPCLS>, std::vector< boost::shared_ptr<HeliDecAmps> >, pawian::Collection::SharedPtrLess >::iterator it;
  ...
  for (int lamHigestJFsp=-_highestJFsp; lamHigestJFsp<=_highestJFsp; lamHigestJFsp=lamHigestJFsp+lamSteps){
    ...
    for(it=_decAmpsSinglet.begin(); it!=_decAmpsSinglet.end(); ++it){
      complex<double> tmpAmp(0.,0.);
      boost::shared_ptr<const JPCLS> theJPCLS=it->first;
      std::vector<boost::shared_ptr<HeliDecAmps> > decAmps=it->second;    
      std::vector<boost::shared_ptr<HeliDecAmps> >::iterator itDec;
      for( itDec=decAmps.begin(); itDec!=decAmps.end(); ++itDec){
...
void pbarpHeliLh::getDefaultParams(fitParams& fitVal, fitParams& fitErr){ 
  ...
  std::vector< boost::shared_ptr<HeliDecAmps> >::iterator itDecs;
...
void  pbarpHeliLh::initialize(){
  ...
  for (it=theDecs.begin(); it!=theDecs.end(); ++it){
    boost::shared_ptr<HeliDecAmps> currentAmp(new HeliDecAmps(*it));
...
void pbarpHeliLh::fillMap(std::vector< boost::shared_ptr<const JPCLS> >& pbarpLSs, std::vector<boost::shared_ptr<HeliDecAmps> >& decs, std::map< boost::shared_ptr<const JPCLS>, std::vector<boost::shared_ptr<HeliDecAmps> >, pawian::Collection::SharedPtrLess >& toFill){


  std::vector< boost::shared_ptr<const JPCLS> >::const_iterator itJPCLS;
  for (itJPCLS = pbarpLSs.begin(); itJPCLS != pbarpLSs.end(); ++itJPCLS){
    std::vector<boost::shared_ptr<HeliDecAmps> > currentAmpVec;

    std::vector<boost::shared_ptr<HeliDecAmps> >::iterator itAmp;
...
void pbarpHeliLh::checkParamVariation(fitParams& theParamVal){
  std::vector< boost::shared_ptr<HeliDecAmps> >::iterator it;
  for (it=_decAmps.begin(); it!=_decAmps.end(); ++it){
    (*it)->checkRecalculation(theParamVal);
  }
}

void pbarpHeliLh::cacheTheAmps(){
  std::vector< boost::shared_ptr<HeliDecAmps> >::iterator it;
  for (it=_decAmps.begin(); it!=_decAmps.end(); ++it){
    (*it)->cacheAmplitudes();
  }
...
void pbarpHeliLh::updateFitParams(fitParams& theParamVal){
