...
void EffCorrectionCompare(std::string rootFileName, std::string histname, bool createcanvas=true);
void EffCorrectionComp2(TH1F* histoData, TH1F* histoFit);
void EffCorrectionCompareAll(std::string rootFileName, bool saveImage=false);
...
void EffCorrectionCompare(std::string rootFileName, std::string histname, bool createcanvas){
  TFile* tFile= new TFile(rootFileName.c_str());
   std::string dataName = "Data" + histname;
   std::string mcName = "Fit" + histname;
   std::string truthName = "TruthWWeight" + histname;
   ...
   if(createcanvas){
     ...
     canvas->cd(3);
     effCorrDataH1->Draw();
     histoTruthH1->Draw("same"); 
   }

   else EffCorrectionComp2(effCorrDataH1, histoTruthH1);

   // if(createcanvas)
   //   //     TCanvas* canvas = new TCanvas("canvas","c1",1000,1000);

   std::cout << "\n" << std::endl;
   CalcHistoChi2(effCorrDataH1, histoTruthH1);
   CalcKolmogorov(effCorrDataH1, histoTruthH1);
   std::cout << "\n" << std::endl;
}
...
void EffCorrectionCompareAll(std::string rootFileName, bool saveImage){
  TFile* tFile= new TFile(rootFileName.c_str());   
   ...
   for(int i=0; i<numhistsMasses; i++){
      canvasMass->cd(i+1);
      EffCorrectionCompare(rootFileName, histnamesMasses.at(i), false);
   }
