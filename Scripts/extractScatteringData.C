...
void extractScatteringData(std::string iFileName="PiPiData.txt"){
  ...
  TCanvas* canvas2 = new TCanvas("Argand","Atgand",20, 20, 900,900);
  canvas2->cd();
  argandGraph->Draw("ALP");  
}
...
double errorSqrT11(double eta, double phase, double deltaeta, double deltaphase){
  ...
  double result=sqrt(dT11overdEta*dT11overdEta*deltaeta*deltaeta+dT11overdPhase*dT11overdPhase*deltaphase*deltaphase);
  return result;
}
