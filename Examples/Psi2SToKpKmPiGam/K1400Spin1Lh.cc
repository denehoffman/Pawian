...
K1400Spin1Lh::K1400Spin1Lh(boost::shared_ptr<const Psi2SToKpKmPiGamEventList> theEvtList, boost::shared_ptr<const Psi2SToKpKmPiGamStates> theStates) :
  AbsPsi2SToKpKmPiGamLh(theEvtList,theStates)  
{
}

K1400Spin1Lh::K1400Spin1Lh( boost::shared_ptr<AbsPsi2SToKpKmPiGamLh> theLhPtr ) :
  AbsPsi2SToKpKmPiGamLh(theLhPtr->getEventList(), theLhPtr->getPsi2SToKpKmPiGamStates())
{
}

K1400Spin1Lh::~K1400Spin1Lh()
{;
}

complex<double> K1400Spin1Lh::calcCoherentAmp(Spin Minit, Spin lamGam, const Psi2SToKpKmPiGamData::fitParamVal& theParamVal, Psi2SToKpKmPiGamData::Psi2SToKpKmPiGamEvtData* theData){
  ...
  double K1400Mass=theParamVal.BwK1400_1.first;
  double K1400Width=theParamVal.BwK1400_1.second;
...
//  	complex<double> ChiToA980pi0Amp=a980Amp(theData, ChiToa0Pi, a980Mass, a980Width, lamChi);
	...
	complex<double> ChiToK1400_1_KAmp=K1_1400Amp(theData, ChiToK1400_1_K, K1400Mass, K1400Width, lamChi);
	...
	PsiDecAmpTmpls+=thePsiMag*expiphiPsi*sqrt(2*PsiState->L+1)
	  ...
	  *(ChiToA980pi0Amp+ChiToK890KAmp+ChiToK1400_1_KAmp);
      }
...
void K1400Spin1Lh::setMnUsrParams(MnUserParameters& upar, Psi2SToKpKmPiGamData::fitParamVal& startVal,  Psi2SToKpKmPiGamData::fitParamVal& errVal){
...
int K1400Spin1Lh::setFitParamVal(Psi2SToKpKmPiGamData::fitParamVal& theParamVal, const std::vector<double>& par) const{
  ...
  if (par.size()!= JPCLSPsiToChiGam.size()*2-1+JPCLSChiToKst1K.size()*4+JPCLSChiToa980pi.size()*2+7) {
    Alert << "size of parameters wrong!!! par.size()=" << par.size() << 
      "\t it should be" << JPCLSPsiToChiGam.size()*2+1+JPCLSChiToKst1K.size()*4+JPCLSChiToa980pi.size()*2+7 << endmsg;
    exit(1);
  }
...
void K1400Spin1Lh::print(std::ostream& os) const{
  os << "K1400Spin1Lh::print\n";
}

void K1400Spin1Lh::printCurrentFitResult(Psi2SToKpKmPiGamData::fitParamVal& theParamVal) const{
  ...
  for ( itJPCLS=JPCLSChiToa980pi.begin(); itJPCLS!=JPCLSChiToa980pi.end(); ++itJPCLS){
    ...
    std::pair<double, double> tmpParamK1400=theParamVal.BwK1400_1;
