...
K1400Spin12Lh::K1400Spin12Lh(boost::shared_ptr<const Psi2SToKpKmPiGamEventList> theEvtList, boost::shared_ptr<const Psi2SToKpKmPiGamStates> theStates) :
  AbsPsi2SToKpKmPiGamLh(theEvtList,theStates)  
{
}

K1400Spin12Lh::K1400Spin12Lh( boost::shared_ptr<AbsPsi2SToKpKmPiGamLh> theLhPtr ) :
  AbsPsi2SToKpKmPiGamLh(theLhPtr->getEventList(), theLhPtr->getPsi2SToKpKmPiGamStates())
{
}

K1400Spin12Lh::~K1400Spin12Lh()
{;
}

complex<double> K1400Spin12Lh::calcCoherentAmp(Spin Minit, Spin lamGam, const Psi2SToKpKmPiGamData::fitParamVal& theParamVal, Psi2SToKpKmPiGamData::Psi2SToKpKmPiGamEvtData* theData){
...
//       complex<double> ChiToA980pi0Amp=a980Amp(theData, ChiToa0Pi, a980Mass, a980Width, lamChi);
	...
	*(ChiToA980pi0Amp+ChiToK890KAmp+ChiToK1400_1_KAmp+ChiToK1400_2_KAmp);
...
void K1400Spin12Lh::setMnUsrParams(MnUserParameters& upar, Psi2SToKpKmPiGamData::fitParamVal& startVal,  Psi2SToKpKmPiGamData::fitParamVal& errVal){
...
int K1400Spin12Lh::setFitParamVal(Psi2SToKpKmPiGamData::fitParamVal& theParamVal, const std::vector<double>& par) const{
  ...
  if (par.size()!= JPCLSPsiToChiGam.size()*2-1+JPCLSChiToKst1K.size()*4+JPCLSChiToKst2K.size()*2+JPCLSChiToa980pi.size()*2+9) {
    Alert << "size of parameters wrong!!! par.size()=" << par.size() << 
      "\t it should be" << JPCLSPsiToChiGam.size()*2+1+JPCLSChiToKst1K.size()*4+JPCLSChiToKst2K.size()*2+JPCLSChiToa980pi.size()*2+9 << endmsg;
    exit(1);
  }
...
void K1400Spin12Lh::print(std::ostream& os) const{
  os << "K1400Spin12Lh::print\n";
}

void K1400Spin12Lh::printCurrentFitResult(Psi2SToKpKmPiGamData::fitParamVal& theParamVal) const{
