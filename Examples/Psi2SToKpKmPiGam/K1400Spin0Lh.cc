...
K1400Spin0Lh::K1400Spin0Lh(boost::shared_ptr<const Psi2SToKpKmPiGamEventList> theEvtList, boost::shared_ptr<const Psi2SToKpKmPiGamStates> theStates) :
  AbsPsi2SToKpKmPiGamLh(theEvtList,theStates)  
{
}

K1400Spin0Lh::K1400Spin0Lh( boost::shared_ptr<AbsPsi2SToKpKmPiGamLh> theLhPtr ) :
  AbsPsi2SToKpKmPiGamLh(theLhPtr->getEventList(), theLhPtr->getPsi2SToKpKmPiGamStates())
{
}

K1400Spin0Lh::~K1400Spin0Lh()
{;
}

complex<double> K1400Spin0Lh::calcCoherentAmp(Spin Minit, Spin lamGam, const Psi2SToKpKmPiGamData::fitParamVal& theParamVal, Psi2SToKpKmPiGamData::Psi2SToKpKmPiGamEvtData* theData){
  ...
  std::map< boost::shared_ptr<const JPCLS>, pair<double, double>, pawian::Collection::SharedPtrLess > ChiToK0_1400K=theParamVal.ChiToK1400_0_K;
  ...
  double K1400Mass=theParamVal.BwK1400_0.first;
  double K1400Width=theParamVal.BwK1400_0.second;
...
//  	complex<double> ChiToA980pi0Amp=a980Amp(theData, ChiToa0Pi, a980Mass, a980Width, lamChi);
	...
	//Chi -> K1400 K and K1400->K pi0
        complex<double> ChiToK0_1400_KAmp=K0_1400Amp(theData, ChiToK0_1400K, K1400Mass, K1400Width, lamChi);
	...
	PsiDecAmpTmpls+=thePsiMag*expiphiPsi*sqrt(2*PsiState->L+1)
          ...
          *(ChiToA980pi0Amp+ChiToK890KAmp+ChiToK0_1400_KAmp);
...
void K1400Spin0Lh::setMnUsrParams(MnUserParameters& upar, Psi2SToKpKmPiGamData::fitParamVal& startVal,  Psi2SToKpKmPiGamData::fitParamVal& errVal){
...
int K1400Spin0Lh::setFitParamVal(Psi2SToKpKmPiGamData::fitParamVal& theParamVal, const std::vector<double>& par) const{
  ...
  if (par.size()!= JPCLSPsiToChiGam.size()*2-1+JPCLSChiToKst1K.size()*2+JPCLSChiToKst0K.size()*2+JPCLSChiToa980pi.size()*2+7) {
    Alert << "size of parameters wrong!!! par.size()=" << par.size() << 
      "\t it should be" << JPCLSPsiToChiGam.size()*2+JPCLSChiToKst1K.size()+1+JPCLSChiToKst0K.size()*4+JPCLSChiToa980pi.size()*2+7 << endmsg;
    exit(1);
  }
...
void K1400Spin0Lh::print(std::ostream& os) const{
  os << "K1400Spin0Lh::print\n";
}

void K1400Spin0Lh::printCurrentFitResult(Psi2SToKpKmPiGamData::fitParamVal& theParamVal) const{
  ...
  for ( itJPCLS=JPCLSChiToKst0K.begin(); itJPCLS!=JPCLSChiToKst0K.end(); ++itJPCLS){
    DebugMsg<< (*itJPCLS)->name()<< "K1400Pi" << endmsg;
  ...
  for ( itJPCLS=JPCLSChiToa980pi.begin(); itJPCLS!=JPCLSChiToa980pi.end(); ++itJPCLS){
    ...
    std::pair<double, double> tmpParamK1400=theParamVal.BwK1400_0;
    DebugMsg <<"\t mass:" << tmpParamK1400.first <<"\t width:" << tmpParamK1400.second  << endmsg;
