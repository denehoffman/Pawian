...
void ROOTDataReader::resetSource() {

  cout << "Resetting source " << m_inTree->GetName() << " in "
       << m_inFile->GetName() << endl;

  // this will cause the read to start back at event 0
  m_eventCounter = 0;
}

Kinematics *ROOTDataReader::getEvent() {
  if (m_eventCounter < static_cast<unsigned int>(m_inTree->GetEntries())) {
    ...
    particleList.push_back(
        TLorentzVector(m_pxBeam, m_pyBeam, m_pzBeam, m_eBeam));
...
unsigned int ROOTDataReader::numEvents() const {
