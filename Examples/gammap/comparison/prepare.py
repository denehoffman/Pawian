...
initialize ksks::PosRe::S0+ cartesian 1.0 0.0 fixed
initialize ksks::PosIm::S0+ cartesian 1.0 0.0 fixed
...
def prepare_parameters(defaults: Path, output: Path) -> list[str]:
    ...
    for raw in defaults.read_text(encoding='utf-8').splitlines():
        ...
        if name == f'{s_stem}Mag':
            ...
            value, step = 0.35, 0.2
        elif 'ToKshort1_Kshort2Mag' in name or name.endswith('Scaling'):
            value = 1.0
        rows.append(f'{name}\t{value:.17g}\t{step:.17g}')
        if name not in free:
            fixed.append(name)
    output.write_text('\n'.join(rows) + '\n', encoding='utf-8')
    return fixed
