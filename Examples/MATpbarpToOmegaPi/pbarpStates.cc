...
vector<LS> pbarpStates::myGetValidLS(const serSpin &__j,int __parity,const serSpin &__s1,int __p1,
...
bool pbarpStates::calcJPCs(){
  for (int j=0; j<=_jmax; j++)
    {
      for (int p=-1; p<=1; p+=2){
       vector<LS> motherLSs=myGetValidLS(j, p, _pbarJPC.J,_pbarJPC.P, _pJPC.J, _pJPC.P);
