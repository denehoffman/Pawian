...
EtaDecAmp::EtaDecAmp(const std::string& name, const std::vector<std::string>& hypVec, boost::shared_ptr<JpsiGamEtaPiPiStates> theStates) :
  XDecAmpBase(name, hypVec, theStates, Spin(0))
{
}
...
complex<double> EtaDecAmp::XdecAmp(Spin lamX, EvtData* theData, fitParamsNew& theParamVal){
...
void  EtaDecAmp::getDefaultParams(fitParamsNew& fitVal, fitParamsNew& fitErr){

  if(_a980piHyp){

    std::vector< boost::shared_ptr<const JPCLS> > a0PiStates=_theStatesPtr->EtaToa0PiStates();
  ...
  if(_sigmaEtaHyp){
    std::vector< boost::shared_ptr<const JPCLS> > f1EtaStates=_theStatesPtr->EtaTof0EtaStates();
    std::map< boost::shared_ptr<const JPCLS>, double, pawian::Collection::SharedPtrLess > currentMagValMap;
    std::map< boost::shared_ptr<const JPCLS>, double, pawian::Collection::SharedPtrLess > currentPhiValMap;
    std::map< boost::shared_ptr<const JPCLS>, double, pawian::Collection::SharedPtrLess > currentMagErrMap;
    std::map< boost::shared_ptr<const JPCLS>, double, pawian::Collection::SharedPtrLess > currentPhiErrMap;
...
void EtaDecAmp::print(std::ostream& os) const{
  return; //dummy
}
