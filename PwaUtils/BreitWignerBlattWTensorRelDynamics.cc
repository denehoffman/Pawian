...
BreitWignerBlattWTensorRelDynamics::BreitWignerBlattWTensorRelDynamics(std::string& name, std::vector<Particle*>& fsParticles, Particle* mother, double massSumDaughter1, double massSumDaughter2, double qR) :
...
complex<double> BreitWignerBlattWTensorRelDynamics::eval(EvtData* theData, AbsXdecAmp* grandmaAmp, Spin OrbMom){
  ...
  complex<double> result=BreitWignerFunction::BlattWTensorRel(orbMom, theData->DoubleString.at(_dynKey), _currentMass, _currentWidth, _fsp1Mass, _fsp2Mass, _qR);  
  if ( _cacheAmps){
     theMutex.lock();
